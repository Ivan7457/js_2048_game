 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    moveUp() {
        ...
        for(let c = 0; c < 4; c++){
            const col = [];
            for(let r = 0; r < 4; r++)col.push(this.board[r][c]);
            transposed.push(col);
        }
        for(let i = 0; i < 4; i++){
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++)this.board[r][c] = transposed[c][r];
        this.afterMove(change);
    }
    moveDown() {
        if (this.status !== "playing") return;
    ...
    addRandomTile() {
        ...
        for(let a = 0; a < this.board.length; a++){
            for(let b = 0; b < this.board[a].length; b++)if (this.board[a][b] === 0) arr.push([
                a,
                b
            ]);
        }
    ...
    processLineLeft(line) {
        ...
        let gained = 0;
        for(let i = 0; i < compact.length - 1; i++)if (compact[i] === compact[i + 1]) {
            compact[i] *= 2;
            gained += compact[i];
            compact[i + 1] = 0;
            i++;
        ...
        while(newCompact.length < 4)newCompact.push(0);
        return {
            newLine: newCompact,
            gained
        };
   ...
   */ restart() {
    ...
    afterMove(didChange) {
        ...
        this.addRandomTile();
        this.checkWin();
        if (this.status !== "win" && !this.isMovePossible()) this.status = "lose";
    }
}
module.exports = Game;
